...
Deep Research Agent implementation using ClaudeSDKClient.
Based on specification v2.0 from tasks/deep_research_agent_spec.md

This implementation uses ClaudeSDKClient (instead of query()) to provide:
- Explicit connection lifecycle management
- Stateful conversation management
- Potential for multi-turn research interactions
- Better session control

Core functionality identical to deep_research_agent_full.py, but with:
...
from claude_agent_sdk.types import AgentDefinition, AssistantMessage, ResultMessage, SystemMessage
...
class DeepResearchAgent:
    ...
    - Specialized subagents (Researcher, Data Manager, Synthesizer)
    - Optional Appwrite MCP integration
    - Explicit connection lifecycle with ClaudeSDKClient
    """
    ...
    def __init__(self, mcp_config_path: Optional[str] = None, persist_workspace: bool = True):
        ...
        logger.info("=" * 80)
        logger.info("[Agent] Initializing DeepResearchAgent (ClaudeSDKClient version)")
...
- TodoWrite updates showing synthesis progress''',
    ...
    def _process_message(self, message: Any) -> None:
        ...
        if isinstance(message, AssistantMessage):
            ...
            # Track usage/cost if available - handle both dict and object formats
            if hasattr(message, 'usage') and message.usage:
                ...
                logger.debug(f"[Cost] Message cost: ${cost:.4f} | Total: ${self.total_cost:.4f}")
    ...
    async def execute(
        ...
        """
        Execute deep research agent with isolated filesystem using ClaudeSDKClient.
        ...
        logger.info("\n" + "=" * 80)
        logger.info("[Execution] Starting Deep Research Session (ClaudeSDKClient)")
        logger.info("=" * 80)
        ...
        with IsolatedFilesystem(self.execution_id, persist=self.persist_workspace) as filesystem:
            ...
            logger.info("[Execution] Creating ClaudeSDKClient...")
            ...
            try:
                ...
                async with client:
                    logger.info("[Execution] Connecting to Claude...")

                    # Connect to Claude
                    await client.connect()
                    logger.info("[Execution] ✓ Connected successfully")

                    # Send research query
                    logger.info("[Execution] Sending research query...")
                    await client.query(task)
                    logger.info("[Execution] ✓ Query sent, processing messages...")

                    # Process messages until completion
                    async for message in client.receive_messages():
                        message_type = type(message).__name__

                        # Print RAW unfiltered message (same as query-based version)
                        logger.info(f"[RAW MESSAGE] Type: {message_type}")
                        logger.info(f"[RAW MESSAGE] Content: {message}")

                        # Process message (track todos, costs, etc.)
...
async def main():
    """Example: Deep research task with ClaudeSDKClient"""

    # Initialize agent
    logger.info("\n" + "🔬 " * 20)
    logger.info("Deep Research Agent - ClaudeSDKClient Implementation")
    logger.info("🔬 " * 20 + "\n")
    ...
    user_query = '''
Research: "Using Claude Agent SDK with python using ClaudeSDKClient"

...
async def upsert_sow_to_appwrite(
    ...
    3. [FORCE MODE] If existing_sow_id provided, delete old SOW and re-link templates
    4. Transform to Appwrite schema:
       ...
       - Add courseId, version (from param), status="draft"
    5. Create document in default.Authored_SOW (ID auto-generated by Appwrite)
    6. [FORCE MODE] Update lesson templates to point to new SOW
    ...
    logger.info(f"  Force mode: {'YES' if existing_sow_id else 'NO'}")

    if existing_sow_id:
        logger.info(f"  → Will overwrite existing SOW: {existing_sow_id}")
    ...
    if existing_sow_id:
        ...
        try:
            ...
            templates_to_relink = await list_appwrite_documents(
                database_id="default",
                collection_id="lesson_templates",
                queries=[f'equal("authored_sow_id", "{existing_sow_id}")'],
            ...
            logger.info(f"  ✓ Old SOW version {version} deleted successfully")

        except Exception as e:
            error_msg = f"Failed to delete existing SOW in force mode: {e}"
    ...
    logger.info(f"🔍 Accessibility Notes Debug (raw):")
    ...
    if isinstance(accessibility_notes_raw, list):
        ...
        accessibility_notes = "\n".join(f"• {item}" for item in accessibility_notes_raw)
        logger.info(f"  ✓ Converted array to string with bullet points")
    elif isinstance(accessibility_notes_raw, str):
        accessibility_notes = accessibility_notes_raw
        logger.info(f"  ✓ Already a string, using as-is")
    else:
        # Fallback: convert to string
        accessibility_notes = str(accessibility_notes_raw)
        logger.info(f"  ⚠️  Unexpected type, converted to string")
    ...
    logger.info(f"🔍 Document Data Debug:")
    ...
    try:
        ...
        new_sow_id = result['$id']

        # Step 7: FORCE MODE - Re-link lesson templates to new SOW
        if existing_sow_id and len(templates_to_relink) > 0:
            logger.info(f"📝 Re-linking {len(templates_to_relink)} lesson template(s) to new SOW...")

            from .appwrite_mcp import update_appwrite_document

            for idx, template in enumerate(templates_to_relink, 1):
                ...
                try:
                    await update_appwrite_document(
                        ...
                        data={"authored_sow_id": new_sow_id},
                        mcp_config_path=mcp_config_path
                    )

                    logger.info(f"  ✓ [{idx}/{len(templates_to_relink)}] Re-linked: {template_id[:12]}... ({template_title})")

                except Exception as e:
                    logger.error(f"  ✗ [{idx}/{len(templates_to_relink)}] Failed to re-link {template_id}: {e}")
                    # Continue with other templates even if one fails

            logger.info(f"✅ Re-linking complete: {len(templates_to_relink)} template(s) now reference new SOW")

        return new_sow_id

    except Exception as e:
        logger.error(f"❌ Appwrite upsert failed!")

"""Custom state schema for Lesson Author Agent with todos reducer.
...
def todos_reducer(left: list[Todo] | None, right: list[Todo] | None) -> list[Todo] | None:
    ...
    Example:
        critic_1: {"todos": [{"content": "Fix CFU", "status": "pending"}]}
        critic_2: {"todos": [{"content": "Add context", "status": "pending"}]}
        Result: {"todos": [
            {"content": "Fix CFU", "status": "pending"},
...
class LessonAuthorState(DeepAgentState):
    ...
    todos field. When multiple subagents (particularly the 5 critic subagents)
    update the todos list concurrently within the same execution step, the reducer
    merges the updates instead of throwing an InvalidUpdateError.

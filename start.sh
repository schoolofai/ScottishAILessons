...
if [ ! -f "$SCRIPT_DIR/langgraph-agent/.git" ] || [ ! -f "$SCRIPT_DIR/langgraph-generic-chat/.git" ]; then
    echo -e "${YELLOW}⚠️  Submodules not initialized. Initializing now...${NC}"
...
# Set up main backend (langgraph-agent)
echo -e "${GREEN}🐍 Setting up main backend...${NC}"
cd "$SCRIPT_DIR/langgraph-agent"

# Create virtual environment if needed
if [ ! -d ".venv" ]; then
    echo -e "${YELLOW}Creating Python virtual environment for main backend...${NC}"
    python3 -m venv .venv
fi
...
if ! pip show langgraph-cli &> /dev/null; then
    echo -e "${YELLOW}Installing main backend dependencies...${NC}"
    pip install -e . "langgraph-cli[inmem]" &> "$SCRIPT_DIR/logs/backend-install.log"
...
langgraph dev &> "$SCRIPT_DIR/logs/backend.log" &
BACKEND_PID=$!
...
# Create virtual environment if needed (separate from main backend)
if [ ! -d ".venv" ]; then
    echo -e "${YELLOW}Creating Python virtual environment for context chat backend...${NC}"
    python3 -m venv .venv
fi
...
if ! pip show langgraph-cli &> /dev/null; then
    ...
    pip install -e . "langgraph-cli[inmem]" &> "$SCRIPT_DIR/logs/context-chat-install.log"
...
langgraph dev --port 2700 &> "$SCRIPT_DIR/logs/context-chat.log" &
CONTEXT_CHAT_PID=$!
...
cd "$SCRIPT_DIR/assistant-ui-frontend"

if [ ! -d "node_modules" ]; then
    echo -e "${YELLOW}Installing frontend dependencies...${NC}"
    npm install --legacy-peer-deps &> "$SCRIPT_DIR/logs/frontend-install.log"
...
npm run dev -- -p 3000 &> "$SCRIPT_DIR/logs/frontend.log" &
FRONTEND_PID=$!
...
echo -e "${YELLOW}Logs: logs/backend.log, logs/context-chat.log, logs/frontend.log${NC}"
echo ""

# Keep script running
echo -e "${GREEN}Servers are running successfully!${NC}"
echo -e "${YELLOW}To view logs in real-time, open a new terminal and run:${NC}"
echo -e "${BLUE}  tail -f logs/backend.log${NC}         (for main backend logs)"
echo -e "${BLUE}  tail -f logs/context-chat.log${NC}   (for context chat backend logs)"
echo -e "${BLUE}  tail -f logs/frontend.log${NC}       (for frontend logs)"
echo ""
echo -e "${YELLOW}The application will keep running until you press Ctrl+C${NC}"
echo ""

# Optional: Show last few lines of logs
if [ -f "$SCRIPT_DIR/logs/backend.log" ] || [ -f "$SCRIPT_DIR/logs/context-chat.log" ] || [ -f "$SCRIPT_DIR/logs/frontend.log" ]; then
    echo -e "${GREEN}Recent log entries:${NC}"

    if [ -f "$SCRIPT_DIR/logs/backend.log" ]; then
        echo -e "${BLUE}--- Main Backend (last 3 lines) ---${NC}"
        tail -3 "$SCRIPT_DIR/logs/backend.log" 2>/dev/null || echo "No main backend logs yet"
        echo ""
    fi

    if [ -f "$SCRIPT_DIR/logs/context-chat.log" ]; then
        echo -e "${BLUE}--- Context Chat Backend (last 3 lines) ---${NC}"
        tail -3 "$SCRIPT_DIR/logs/context-chat.log" 2>/dev/null || echo "No context chat backend logs yet"
        echo ""
    fi

    if [ -f "$SCRIPT_DIR/logs/frontend.log" ]; then
        echo -e "${BLUE}--- Frontend (last 3 lines) ---${NC}"
        tail -3 "$SCRIPT_DIR/logs/frontend.log" 2>/dev/null || echo "No frontend logs yet"
        echo ""
    fi
fi

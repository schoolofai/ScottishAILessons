...
**API Endpoint:** `/api/v1/render/desmos/simple`
...
**API Endpoint:** `/api/v1/render/geogebra/simple`
...
**API Endpoint:** `/api/v1/render` (legacy base endpoint)

**Note:** This is the legacy renderer. For most new implementations, consider using GeoGebra (for geometry) or Desmos (for coordinate-based function work). JSXGraph remains useful for specific coordinate transformation scenarios.
...
**JSXGRAPH limitations:**
- Legacy API, less documented than newer tools
- Consider GeoGebra as alternative for most geometry
- No `/simple` endpoint - requires full diagram specification
...
**API Endpoint:** `/api/v1/render/plotly`
...
**API Endpoint:** `/api/v1/render/imagen`
**Model:** `gemini-3-pro-image-preview`
**Rate Limit:** 10 requests per minute

**Select GEMINI_IMAGE when the request involves:**
...
```json
{
  "tool": "DESMOS" | "GEOGEBRA" | "JSXGRAPH" | "PLOTLY" | "GEMINI_IMAGE" | "NONE",
...
| Classifier Output | API Endpoint | Notes |
|-------------------|--------------|-------|
| `DESMOS` | `/api/v1/render/desmos/simple` | Simple expression format |
| `GEOGEBRA` | `/api/v1/render/geogebra/simple` | Simple command format |
| `JSXGRAPH` | `/api/v1/render` | Legacy, full diagram spec required |
| `PLOTLY` | `/api/v1/render/plotly` | Chart data format |
| `GEMINI_IMAGE` | `/api/v1/render/imagen` | AI image generation |
| `NONE` | N/A | No visualization needed |
...
4. **If the request involves transformations or vectors on coordinates** → JSXGRAPH (or GEOGEBRA)
5. **If the request describes a real-world physical scenario for context** → GEMINI_IMAGE
6. **If the request involves angles/bearings/geometry WITHOUT coordinates** → GEOGEBRA
7. **If the request involves lines/points WITH specific coordinates** → JSXGRAPH or GEOGEBRA
...
| "Draw triangle ABC at coordinates..." | JSXGRAPH or GEOGEBRA |
| Straight line y = mx + c | DESMOS |
| Gradient between two points | JSXGRAPH or GEOGEBRA |
| Trig graphs (sin, cos, tan) | DESMOS |
| Trig in right-angled triangles | GEOGEBRA (or GEMINI_IMAGE for context) |
...
| "Visualize the problem" (word problem) | GEMINI_IMAGE |
...
- **Trigonometry (Triangles):** GEOGEBRA + GEMINI_IMAGE for context
- **Geometry (Circle):** GEOGEBRA
- **Geometry (Transformations):** JSXGRAPH or GEOGEBRA
- **Vectors:** JSXGRAPH or GEOGEBRA
- **Statistics:** PLOTLY
- **Coordinate Geometry:** JSXGRAPH, GEOGEBRA, or DESMOS depending on focus
